...
void main() {
	...
	ivec2 p0 = ivec2( int( RangeRemapValue( line.p0.x, -ratio, ratio, 0.0f, iS.x ) ), int( RangeRemapValue( line.p0.y, -1.0f, 1.0f, 0.0f, iS.y ) ) );
	// uint d0 = uint( RangeRemapValue(  ) ); // depths
	ivec2 p1 = ivec2( int( RangeRemapValue( line.p1.x, -ratio, ratio, 0.0f, iS.x ) ), int( RangeRemapValue( line.p1.y, -1.0f, 1.0f, 0.0f, iS.y ) ) );
	// uint d1 = uint( RangeRemapValue(  ) );

	// draw the line
	plotLineWidth( p0.x, p0.y, p1.x, p1.y, line.color, 1.0 );
}