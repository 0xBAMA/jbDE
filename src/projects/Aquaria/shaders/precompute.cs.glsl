...
void main () {
	...
	const vec3 pos = vec3( writeLoc ) + vec3( 0.5f ) - imageSize( dataCacheBuffer ) / 2.0f;
	...
	for ( int i = 0; i < numSpheres; i++ ) {
		...
		// minDistance = min( minDistance, distance( pos, pr.xyz ) - pr.w );
	...
	// imageStore( dataCacheBuffer, writeLoc, vec4( color, 1.0f ) );
