...
void main() {
	...
	// glFragColor = vec4( tRead.xyz * color * clamp( dot( normal, ( rot * vec3( 1.0f ) ) - worldPosition ), 0.1618f, 1.0f ) * 2.5f, 1.0f );
	// glFragColor = vec4( tRead.xyz * color * clamp( dot( normal, worldPosition - rot * vec3( 1.0f ) ), 0.1618f, 1.0f ) * 2.5f, 1.0f );
	// glFragColor = vec4( tRead.xyz * color * clamp( dot( normal, worldPosition - vec3( 1.0f, 1.0f, -1.0f ) ), 0.1618f, 1.0f ) * 1.3f, 1.0f );
