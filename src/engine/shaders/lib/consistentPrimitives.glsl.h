...
float iSphere( in vec3 ro, in vec3 rd, inout vec3 normal, float sphereRadius ) {
	float b = dot(ro, rd);
...
float iBox( in vec3 ro, in vec3 rd, inout vec3 normal, in vec3 boxSize ) {
...
float iCylinder( in vec3 ro, in vec3 rd, inout vec3 normal, in vec3 pa, in vec3 pb, float ra ) {
	vec3 ca = pb-pa;
	vec3 oc = ro-pa;
...
float iTriangle( in vec3 ro, in vec3 rd, inout vec3 normal, in vec3 v0, in vec3 v1, in vec3 v2 ) {
	vec3 v1v0 = v1 - v0;
	vec3 v2v0 = v2 - v0;
	vec3 rov0 = ro - v0;
...
float iGoursat( in vec3 ro, in vec3 rd, inout vec3 normal, in float ra, float rb ) {
...
float iRoundedBox(in vec3 ro, in vec3 rd, inout vec3 normal, in vec3 size, in float rad ) {
	// bounding box
	vec3 m = 1.0/rd;
	vec3 n = m*ro;
